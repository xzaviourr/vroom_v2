...
def run_simulation(pods:List, load:List):
    for pod in pods:
        ...
        create_pod_yaml(pod[3].memory, pod[3].compute, "ts3", 5555, 12348)
        subprocess.run(["kubectl", "apply", "-f", "pod_request.yaml"])
        subprocess.run(["kubectl", "apply", "-f", "pod_service.yaml"])
        ...
        startup_time = measure_start_time(start_time, "ts3", 12348)
        ...
        for num_requests in load:
            ...
            print(f"Running for - mem1:{pod[0].memory}|mem2:{pod[1].memory}|mem3:{pod[2].memory}|mem4:{pod[3].memory}|com1:{pod[0].compute}|com2:{pod[1].compute}|com3:{pod[2].compute}|com3:{pod[3].compute}|load:{num_requests}\n")
            throughput, latency, latencies = asyncio.run(measure_overall_throughput(num_requests, ["ts1", "ts2", "ts3", "ts4"], [12345, 12346, 12347, 12348]))
            
            stop_event.set()
            monitoring_thread.join()

            output_str = f"{pod[0].memory},{pod[0].compute},{pod[1].memory},{pod[1].compute},{pod[2].memory},{pod[2].compute},{pod[3].memory},{pod[3].compute},{round(startup_time,3)},{num_requests},{round(throughput,3)},{round(latency,3)},'{latencies}','{results}'\n"
...
if __name__ == "__main__":
    pods = [
        ...
        [Pod(3, 20), Pod(3, 20), Pod(3, 20), Pod(3, 10)],   # 12, 80
        [Pod(3, 30), Pod(3, 30), Pod(3, 30), Pod(3, 10)],   # 12, 120
        [Pod(3, 40), Pod(3, 40), Pod(3, 40), Pod(3, 10)],   # 12, 160
        [Pod(3, 50), Pod(3, 50), Pod(3, 50), Pod(3, 10)],   # 12, 200
        [Pod(3, 60), Pod(3, 60), Pod(3, 60), Pod(3, 10)],   # 12, 240
        [Pod(3, 70), Pod(3, 70), Pod(3, 70), Pod(3, 10)],   # 12, 280
        [Pod(3, 80), Pod(3, 80), Pod(3, 80), Pod(3, 10)],   # 12, 320
        [Pod(3, 90), Pod(3, 90), Pod(3, 90), Pod(3, 10)],   # 12, 360
        [Pod(3, 100), Pod(3, 100), Pod(3, 100), Pod(3, 10)],   # 12, 400
    ]
    # load = [2, 4, 8, 16, 32, 64, 128]
    load = [128]

    output_str = f"memory1,compute1,memory2,compute2,memory3,compute3,memory4,compute4,startup_time,load,throughput,latency,latencies,utilization\n"
    with open("results.csv", 'a') as file:
        file.write(output_str)
