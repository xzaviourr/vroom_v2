...
def run_simulation(pods:List, load:List, filename:str):
    for pod in pods:
        # Create YAML content
        ...
        for num_requests in load:
            ...
            mem, com = 0, 0
            for ind in range(1, num_colocation+1):
                mem += pod[ind-1].memory
                com += pod[ind-1].compute

            s += f'{num_colocation},{mem},{com},{round(startup_time, 3)},{num_requests},{round(throughput, 3)},{round(latency, 3)},"{latencies}","{results}"\n'
...
if __name__ == "__main__":
    ...
    pods = [
        [Pod(12, 100)],
        [Pod(6, 50), Pod(6, 50)],
        [Pod(4, 33), Pod(4, 33), Pod(4, 33)],
        [Pod(3, 25), Pod(3, 25), Pod(3, 25), Pod(3, 25)],
    ]

    s = "colocation,memory,compute,startup_time,arrival_rate,throughput,average_latency,latencies,utilization\n"
    with open(file_name, 'a') as file:
        file.write(s)

    run_simulation(pods, load, file_name)
    ...
    pods = [
        ...
        [Pod(4, 50), Pod(4, 50), Pod(4, 50), Pod(4, 50)],
        [Pod(3, 40), Pod(3, 40), Pod(3, 40), Pod(3, 40), Pod(3, 40)],
    ]
    ...
    pods = [
        ...
        [Pod(3, 60), Pod(3, 60), Pod(3, 60), Pod(3, 60), Pod(3, 60)],
    ]
