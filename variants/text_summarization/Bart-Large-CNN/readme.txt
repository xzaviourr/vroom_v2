...
List of commands-
    * Build - sudo docker build -t synergcseiitb/bart-large-cnn-text_summarization .
    * Run - sudo docker run -d --gpus all -p 4444:4444 --runtime=nvidia --ipc=host synergcseiitb/bart-large-cnn-text_summarization 
    * Test - 
        Input - curl --location --request POST 'http://localhost:4444/summarize' \
                --header 'Content-Type: application/json' \
                --data-raw '{
